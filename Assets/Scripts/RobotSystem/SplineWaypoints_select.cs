...
public class SplineWaypoints_select : MonoBehaviour
{
    ROSConnection ros;
        ...
        foreach (Transform point in splinePoints)
        {
            if (point != splineObject.transform) // 自身のスプラインオブジェクトを除外
            {
                ...
                else if (point.CompareTag("Spline2_Arrow"))
                {
                    ...
                    // Spline2用の処理を追加
                }else if(point.CompareTag("Spline3_Arrow"))
                {
                    Debug.Log("Processing Spline 3 Arrow");
                    // Spline3用の処理を追加
                }
    ...
    public void SwitchToThiredSpline() // Bボタン用
    {
        SwitchSpline(2);
    }
